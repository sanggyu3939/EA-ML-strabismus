...
def build_models(preprocess, seed: int) -> Dict[str, Pipeline]:
    """
    Pipelines share the same preprocessing to ensure fair comparison.
    """
...
def get_oof_and_fold_scores(
    ...
    Produces pooled out-of-fold predictions.
    """
...
def evaluate_all_models(
    ...
    for name, model in models.items():
        oof, fold_df = get_oof_and_fold_scores(model, X, y, groups, n_splits=n_splits, seed=seed)
        oof_store[name] = oof
    ...
    perf_df = pd.DataFrame(rows).sort_values("AUROC (OOF)", ascending=False).reset_index(drop=True)
    return perf_df, oof_store, fold_store


def pick_best_model(perf_df: pd.DataFrame) -> str:
    """
    Select best model by AUROC (OOF). (Change if you prefer a different criterion.)
    """
    return perf_df.iloc[0]["Model"]
