...
def youden_table(y: np.ndarray, oof_store: Dict[str, np.ndarray], outdir: str) -> pd.DataFrame:
    ...
    for name, p in oof_store.items():
        ...
        youden = tpr - fpr
        t_star = thr[int(np.argmax(youden))]
    ...
    df.to_csv(
        os.path.join(outdir, "Table_Youden_metrics_all_models.csv"),
        index=False,
        encoding="utf-8-sig",
    )
    return df
...
def try_shap_summary_plot(
    ...
    Requires `shap` installed. If SHAP fails, returns None (no hard crash).
    """
    try:
