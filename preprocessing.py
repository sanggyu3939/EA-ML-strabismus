...
def ensure_patient_id(
    ...
    Returns:
      df (possibly copied), resolved_patient_id_col
    """
    ...
    raise ValueError(
        f"Missing patient id column '{patient_id_col}'. Tried fallbacks: {fallback_cols}"
    )
...
def default_feature_spec() -> FeatureSpec:
    """
    Default feature set used in the manuscript.

    NOTE:
      - Only columns present in the provided dataset will be used.
      - Keep these names aligned with the final tables/figures in the paper.
    """
    features = [
        "age",
        "BMI",
        ...
        "URIsx",
        "rocuron_dose_kg",
        "Fentanly_total_dose_intraop",
        "ane_time",
        "op_time",
        "tube1LMA2",
        "ane_drug_K1M2KM3pofol45",
    ]
    cat_cols = [
        "sexM1F2",
        "ASAclass",
...
def build_X_y_groups(
    ...
    Returns:
      ...
      groups: patient id array used for StratifiedGroupKFold
    ...
    if y.isna().any():
        raise ValueError(
            f"Outcome column '{outcome_col}' contains non-numeric or missing values after coercion. "
            f"Please ensure it is coded as 0/1."
        )
...
def build_preprocessor(
    ...
    drop_first=True aligns with a clear reference category in manuscript tables.
    """
